...
docker run -v $OVPN_DATA:/etc/openvpn --log-driver=none --rm kylemanna/openvpn ovpn_genconfig -C 'AES-256-CBC' -a 'SHA384' -u udp://VPN.SERVERNAME.COM
docker run -v $OVPN_DATA:/etc/openvpn -e EASYRSA_KEY_SIZE=4096 --log-driver=none --rm -it kylemanna/openvpn ovpn_initpki
